...
- Last updated: 2026-05-23T14:12:00+07:00.
...
- Commands run:
  ...
  - `graphify update .`: generated local `graphify-out/GRAPH_REPORT.md` with 260 nodes and 283 edges; `graphify-out/` is ignored as generated output.
  ...
  - `.venv\Scripts\python.exe src/benchmark.py --config project_spec.yaml --edges-path data/raw/roadNet-CA.sample200000.txt --graph-name roadNet-CA-sample200k --gpu --versions v2,v3_pull,v3_push --max-iter 20 --no-scipy-verify --output artifacts/roadnet_ca_sample_benchmarks.csv`: V3 pull speedup `559.4924424627166x`.
  ...
  - `.venv\Scripts\python.exe src/benchmark.py --config project_spec.yaml --gpu`: default synthetic_small GPU benchmark, V3 pull speedup `13.893562463636782x`.
  - `.venv\Scripts\python.exe -m pytest -q`: final verify passed, `10 passed`.
  - `.venv\Scripts\python.exe -m pytest tests/ -q`: final verify passed, `10 passed`.
  - `.venv\Scripts\python.exe src/cpu_baseline.py` and `.venv\Scripts\python.exe src/cpu_baseline.py --config project_spec.yaml`: final verify passed, JSON metrics emitted.
  - `.venv\Scripts\python.exe src/benchmark.py --config project_spec.yaml` and `.venv\Scripts\python.exe src/benchmark.py --config project_spec.yaml --gpu`: final verify passed.
...
- Audit status: final PDF compliance audit added in `docs/final_pdf_compliance_audit.md`; overall result is PASS WITH WARNINGS, with full com-Youtube and 5 full SNAP graph benchmark set still pending.
